...
void Q2() {
	int x, y, sum, diff;
	printf("정수 2개 입력 >>"); scanf_s(" %d %d", &x, &y);
...
void get_lcm_gcd(int x, int y, int* p_lcm, int* p_gcd) {
	...
	// 결국, 12로 다 묶이므로 최대공약수 12

	int a = x, b = y, c;
	while (1) {
		c = a % b; // 모듈로 연산 결과
		if (c == 0) { *p_gcd = b; break; } // 0이면 나눈 수가 최대 공약수
		else a = b, b = c; // 0이 아니면(나눈 수, 나머지)로 다시 반복
	}
	*p_lcm = (x * y) / *p_gcd;
}
void Q10() {
	int x, y, p_lcm, p_gcd;
	printf("두 개의 정수를 입력하시오: "); scanf_s(" %d %d", &x, &y);
...
void get_int(int* px, int* py) {
	printf("2개의 정수를 입력하시오: "); scanf_s(" %d %d", px, py);
}
void Q12() {
	int x, y;
	get_int(&x, &y);
