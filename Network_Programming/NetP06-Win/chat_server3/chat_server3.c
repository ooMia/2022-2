	...
	while (1) {
		...
		for (i = 0; i < num_chat; i++) {
			...
			if (FD_ISSET(client_fds[i], &read_fds)) {
				...
				// buf1 == "/login"
				if (strncmp(buf1, CHAT_CMD_LOGIN, strlen(CHAT_CMD_LOGIN)) == 0) {
				...
				// buf2 == "/list"
				if (strncmp(buf2, CHAT_CMD_LIST, strlen(CHAT_CMD_LIST)) == 0) {
				...
				// buf2 == "/exit"
				if (strncmp(buf2, CHAT_CMD_EXIT, strlen(CHAT_CMD_EXIT)) == 0) {
					RemoveClient(i);
					continue;
				}
